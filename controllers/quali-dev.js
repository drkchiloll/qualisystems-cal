//Objects Needed
var quali    = require('../lib/qualiApi-dev'),
...
var qualiDt = 'MM/DD/YYYY HH:mm',
    ...
    to      = moment().add(1, 'days').format(qualiDt);
  ...
  return Promise.map(reservations, function(reservation) {
    return db.query('temp', reservation.id).then(function(data) {
      if (!data) {
        ...
        // CANCELATION OR OLD?
        var timeOfReservationRequest = moment(dt);
...
var init = function() {
  ...
  .then(function(data) {
    if(data) {
      reservations = modelReservations(JSON.parse(data));
    } else {
